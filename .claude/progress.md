...
- 2026-08-16: Reworked project cards so they can no longer grow unbounded. Collapsed cards show only a new one-line `short` hook (clamped to 2 lines, uniform card heights); clicking a card morphs it to the centre of the viewport over a backdrop to reveal the full `desc`, and clicking again / backdrop / Escape returns it to its slot. New module `js/project-expand.js`; hooks added for all 8 projects.
- 2026-08-16: Fixed paging past the Projects section — a shorter page let scroll anchoring push the reader into Certifications. Grid now sets `overflow-anchor: none` and `js/projects.js` re-frames the filter row under the nav after a page change when needed.
